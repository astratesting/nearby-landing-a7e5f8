...
export default function ListingCard({ listing }: ListingCardProps) {
  const price =
    typeof listing.price === "object" && listing.price.toString
    ...
    >
      ...
      <div className="p-4">
        <div className="flex items-start justify-between gap-2 mb-2">
          <h3
            className="font-manrope font-semibold text-[#1F2937] text-sm leading-snug line-clamp-2"
          >
            {listing.title}
          </h3>
        ...
        <div className="flex items-center gap-2 mb-2">
          ...
          {CONDITION_BADGES[listing.condition] && (
            <span
              className={`text-[11px] px-2 py-0.5 rounded-full font-manrope font-medium ${CONDITION_BADGES[listing.condition].color}`}
            >
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-1 text-xs text-[#1F2937]/50">
            ...
            <span style={{ fontFamily: "'Source Sans 3', sans-serif" }}>{listing.neighborhood}</span>
          </div>
          {listing.seller.trustScore > 0 && (
            <div className="flex items-center gap-1 text-xs text-violet font-semibold">
              ...
              <span style={{ fontFamily: "'Manrope', sans-serif" }}>{listing.seller.trustScore}</span>
            </div>
          )}
        </div>
      </div>
