const steps = [
  ...
  },
  {
    ...
    description:
      "Chat in-app, agree on a neighborhood meeting spot, and exchange items. No phone numbers shared — just trusted, local transactions.",
    icon: (
      <svg width="32" height="32" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round">
    ...
    <section id="how-it-works" className="py-24 px-6 bg-white">
      <div className="max-w-5xl mx-auto">
        <div className="text-center mb-16">
          ...
          <div className="w-12 h-[2px] bg-violet/30 mx-auto mt-6" />
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8 md:gap-10">
          {steps.map((step, i) => (
            <div key={step.number} className="relative text-center px-4">
              {i < steps.length - 1 && (
                <div className="hidden md:block absolute top-16 left-[calc(50%+32px)] right-[calc(-50%+32px)] h-[1px] bg-violet/10" />
              )}

              <div className="relative inline-flex items-center justify-center w-16 h-16 rounded-full border border-violet/15 bg-warm-white mb-6">
                <span
              ...
              <div className="text-violet/60 mb-4 flex justify-center">
                {step.icon}
              </div>

              <h3
                className="text-xl font-semibold text-product-charcoal mb-3"
                style={{ fontFamily: "'Manrope', sans-serif" }}
              >
                {step.title}
              </h3>
              <p
                className="text-sm leading-relaxed text-product-charcoal/55 max-w-xs mx-auto"
